!(function () {
    ...
    CKEDITOR.editorConfig = function (config) {
        ...
        config.toolbar = [
            ...
            {name: 'uimage', items: ['Link', 'Unlink', 'Table', 'UploadImage', 'UploadVideo','UploadFile']},
        ...
        init: function (editor) {
            ...
            editor.addCommand('uimage', {
                exec: function (editor) {
                    ...
                    var url = window.ROOT_URL + '/index.php/admin/plugs/upfile.html?mode=one&type=png,jpg,gif,jpeg,xlsx,doc,pdf,xls,docx,rar,zip&field=' + field;
                    $('<input type="hidden">').attr('name', field).appendTo(editor.element.$).on('change', function () {
                      //  console.log(this);
                        var v = this.value;
                        var vv = v.split('.');
                        v = vv.pop();
                        var f_name = $(this).attr('data-fname');
                        if(v=='png'||v=='jpg'||v=='gif'||v=='jpeg'){
                            var element = CKEDITOR.dom.element.createFromHtml('<img src="' + this.value + '" border="0" title="image" />');
                        }else{
                            var element = CKEDITOR.dom.element.createFromHtml('<a href="' + this.value + '" >'+f_name+'</a>');
                        }
